...
_trueline_font_style() {
    # Sets _trueline_style rather than echoing, so callers don't need a
    # subshell (this runs for every segment on every prompt)
    case "$1" in
        bold)
            _trueline_style=1
            ;;
...
_trueline_content() {
    # An empty color key (e.g. the newline segment has no colors defined)
    # would make bash throw "bad array subscript", so guard the lookups
    fg_c=""
    [[ -n "$1" ]] && fg_c="${TRUELINE_COLORS[$1]}"
    bg_c="$2"
    _trueline_font_style "$3"
    style="$_trueline_style"
    content="$4"
    esc_seq_start="\["
    esc_seq_end="\]"
    if [[ -n "$5" ]] && [[ "$5" == "vi" ]]; then
    ...
    if [[ "$bg_c" != 'default_bg' ]] && [[ -n "$bg_c" ]]; then
...
_trueline_ip_address() {
    ...
    if ip_loc="$(type -p "ip")" || [[ -n $ip_loc ]]; then
        ...
        # Single ifconfig process; parse in bash instead of a
        # grep | grep | head pipeline
        local line
        while IFS= read -r line; do
            if [[ "$line" == *'inet '* && "$line" != *'127.0.0.1'* ]]; then
                read -r _ ip_address _ <<< "$line"
                break
            fi
...
_trueline_user_segment() {
    ...
    local has_ssh=''
    [[ -n "$SSH_CLIENT" || -n "$SSH_TTY" ]] && has_ssh='has_ssh'
    if [[ -n "$has_ssh" ]]; then
        fg_color=${TRUELINE_USER_ROOT_COLORS[0]}
        bg_color=${TRUELINE_USER_ROOT_COLORS[1]}
...
_trueline_git_status() {
    # One `git status --porcelain=v2 --branch --show-stash` call replaces the
    # previous rev-parse + config + rev-list + 3x diff + ls-files + stash-list
    # invocations (one git process instead of eight)
    _trueline_git_branch=''
    _trueline_git_ahead=0
    _trueline_git_behind=0
    _trueline_git_mod=0
    _trueline_git_del=0
    _trueline_git_staged=0
    _trueline_git_untracked=0
    _trueline_git_stash=0
    local status_out
    status_out="$(git status --porcelain=v2 --branch --show-stash 2> /dev/null)"
    if [[ -z "$status_out" ]]; then
        # Either not a git repo, or git < 2.37 (no --show-stash support);
        # retry without it and count stashes the slow way
        status_out="$(git status --porcelain=v2 --branch 2> /dev/null)"
        _trueline_git_stash=$(( $(git stash list 2> /dev/null | wc -l) ))
    fi
    local line
    while IFS= read -r line; do
        case "$line" in
            '# branch.head '*)
                _trueline_git_branch="${line#"# branch.head "}"
                ;;
            '# branch.ab '*)
                # "# branch.ab +<ahead> -<behind>"
                read -r _ _ _trueline_git_ahead _trueline_git_behind <<< "$line"
                _trueline_git_ahead=${_trueline_git_ahead#+}
                _trueline_git_behind=${_trueline_git_behind#-}
                ;;
            '# stash '*)
                _trueline_git_stash="${line#"# stash "}"
                ;;
            '? '*)
                ((_trueline_git_untracked++))
                ;;
            '1 '*|'2 '*)
                # "<n> <XY> ...": X is the staged (index) status, Y the
                # unstaged (worktree) status; the XY field is at offset 2
                local xy=${line:2:2}
                local x=${xy:0:1}
                local y=${xy:1:1}
                [[ "$y" == 'M' ]] && ((_trueline_git_mod++))
                [[ "$y" == 'D' ]] && ((_trueline_git_del++))
                [[ "$x" == 'A' || "$x" == 'M' ]] && ((_trueline_git_staged++))
                ;;
        esac
    done <<< "$status_out"
    [[ "$_trueline_git_branch" == '(detached)' ]] \
        && _trueline_git_branch=$(git rev-parse --short=8 HEAD 2>/dev/null)
}
...
_trueline_git_segment() {
    _trueline_git_status
    if [[ -n "$_trueline_git_branch" ]]; then
        local fg_color="$1"
        local bg_color="$2"
        local font_style="$3"
        local segment="$(_trueline_separator )"
        ...
        segment+="$(_trueline_content "$fg_color" "$bg_color" "$font_style" "${branch_icon}[$_trueline_git_branch] ")"
        local status_part
        if (( _trueline_git_stash > 0 )); then
            status_part="${TRUELINE_SYMBOLS[git_stashed]}"
            [[ "$TRUELINE_GIT_SHOW_STATUS_NUMBERS" = true ]] && status_part+="${_trueline_git_stash}"
            segment+="$(_trueline_content "$TRUELINE_GIT_STASHED_COLOR" "$bg_color" "$font_style" "$status_part ")"
        fi
        if (( _trueline_git_mod > 0 )); then
            status_part="${TRUELINE_SYMBOLS[git_modified]}"
            [[ "$TRUELINE_GIT_SHOW_STATUS_NUMBERS" = true ]] && status_part+="${_trueline_git_mod}"
            segment+="$(_trueline_content "$TRUELINE_GIT_MODIFIED_COLOR" "$bg_color" "$font_style" "$status_part ")"
        fi
        if (( _trueline_git_del > 0 )); then
            status_part="${TRUELINE_SYMBOLS[git_deleted]}"
            [[ "$TRUELINE_GIT_SHOW_STATUS_NUMBERS" = true ]] && status_part+="${_trueline_git_del}"
            segment+="$(_trueline_content "$TRUELINE_GIT_DELETED_COLOR" "$bg_color" "$font_style" "$status_part ")"
        fi
        if (( _trueline_git_untracked > 0 )); then
            status_part="${TRUELINE_SYMBOLS[git_untracked]}"
            [[ "$TRUELINE_GIT_SHOW_STATUS_NUMBERS" = true ]] && status_part+="${_trueline_git_untracked}"
            segment+="$(_trueline_content "$TRUELINE_GIT_UNTRACKED_COLOR" "$bg_color" "$font_style" "$status_part ")"
        fi
        if (( _trueline_git_staged > 0 )); then
            status_part="${TRUELINE_SYMBOLS[git_staged]}"
            [[ "$TRUELINE_GIT_SHOW_STATUS_NUMBERS" = true ]] && status_part+="${_trueline_git_staged}"
            segment+="$(_trueline_content "$TRUELINE_GIT_STAGED_COLOR" "$bg_color" "$font_style" "$status_part ")"
        fi
        local behind_ahead=''
        if (( _trueline_git_behind > 0 )); then
            behind_ahead="${TRUELINE_SYMBOLS[git_behind]}"
            [[ "$TRUELINE_GIT_SHOW_STATUS_NUMBERS" = true ]] && behind_ahead+="${_trueline_git_behind}"
        fi
        if (( _trueline_git_ahead > 0 )); then
            behind_ahead+="${TRUELINE_SYMBOLS[git_ahead]}"
            [[ "$TRUELINE_GIT_SHOW_STATUS_NUMBERS" = true ]] && behind_ahead+="${_trueline_git_ahead}"
        fi
...
_trueline_working_dir_segment() {
    ...
    if [[ "$path_size" -eq 1 ]]; then
        ...
        local path_="${arr[0]:=/}$wd_separator${arr[1]}"
    else
        if [[ "$path_size" -gt 3 ]]; then
            if [[ "$TRUELINE_WORKING_DIR_ABBREVIATE_PARENT_DIRS" = true ]]; then
                # Truncate every parent dir to its first N characters
                # (the last component is left whole)
                local new_p='' comp
                local n=$(( path_size - 1 ))
                for (( i=0; i<n; i++ )); do
                    new_p+="${arr[i]:0:$TRUELINE_WORKING_DIR_ABBREVIATE_PARENT_DIRS_LENGTH}/"
                done
                p="${new_p}${arr[n]}"
            else
                # Keep only the last 3 path components
                p="${TRUELINE_SYMBOLS[working_dir_folder]}/${arr[-3]}/${arr[-2]}/${arr[-1]}"
            fi
        fi
        local curr="${p##*/}"
        p="${p%/*}"
...
_trueline_prompt_command() {
    ...
    local segment_def segment_name segment_fg segment_bg font_style
    for segment_def in "${TRUELINE_SEGMENTS[@]}"; do
        IFS=',' read -r segment_name segment_fg segment_bg font_style <<< "$segment_def"
    ...
    PS1+=$(_trueline_content "$_first_color_fg" default_bg bold "\$")
...
if [[ -z "$TRUELINE_GIT_DELETED_COLOR" ]]; then
    TRUELINE_GIT_DELETED_COLOR='red'
fi
if [[ -z "$TRUELINE_GIT_BEHIND_AHEAD_COLOR" ]]; then
    TRUELINE_GIT_BEHIND_AHEAD_COLOR='purple'
fi
if [[ -z "$TRUELINE_GIT_STASHED_COLOR" ]]; then
    TRUELINE_GIT_STASHED_COLOR='light_blue'
fi
if [[ -z "$TRUELINE_GIT_UNTRACKED_COLOR" ]]; then
    TRUELINE_GIT_UNTRACKED_COLOR='red'
fi
if [[ -z "$TRUELINE_GIT_STAGED_COLOR" ]]; then
